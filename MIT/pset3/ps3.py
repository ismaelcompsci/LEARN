...
def get_word_score(word, n):
    ...
    # first component
    # sum of points for letters in the word
    first_component = 0
    ...
    # 7 x word_len - 3 x (n - word_len ) or 1
    score = 7 * word_len - 3 * (n - word_len)
    second_component = score if score > 1 else 1
...
def update_hand(hand: dict, word: str):
    ...
    # substract letters from dictionary from word
    # dictionary value - word letter
...
update_hand({"j": 2, "o": 1, "l": 1, "w": 1, "n": 2}, "jolly")
...
def is_valid_word(word, hand, word_list):
    ...
    if "*" in word:
        for vowel in VOWELS:
            new_word = word_copy.replace("*", vowel)

            if new_word in word_list:
                word_ = new_word

                if word_ not in word_list:
                    return False
                return True
        ...
        for letter in word:
            ...
            if hand[letter] < word_letter_count[letter]:
                return False
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(user_input, hand, word_list):
                ...
                word_score = get_word_score(user_input, len(hand))
                total_score += word_score

                print(
                    f'"{user_input}" earned {word_score}. Total: {total_score} points'
                )
...
def substitute_hand(hand, letter):
    ...
    while True:
        ...
        if x in hand_copy.keys() or x == letter:
            continue

        del hand_copy[letter]
        hand_copy[x] = hand[letter]
        break
    ...
    * Returns the total score for the series of hands
    ...
    while hands != 0:
        ...
        if not replayed_hand:
            replay = input("Would you like to replay the current hand? ")
            ...
            if replay:
                replayed_hand = True
                current_score_replay = play_hand(current_hand, word_list)

                if current_score > current_score_replay:
                    total_score += current_score
                    hands -= 1
                    continue

                total_score += current_score_replay
        hands -= 1
